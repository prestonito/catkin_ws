...
class CheckAndMoveSM(smach.StateMachine):
    ...
    def __init__(self):
        ...
        #creating publsiher to publish on tts topic
        self.talk = rospy.Publihser('/tts/phrase' String, queue_size = 1)
        ...
        with self:
            ...
            #creating state that says what its doing
            smach.StateMachine.add('SAY_OBSTACLE', TalkingState()),
                                    transitions = {'succeeded':'obstacle'},
                                    remapping = {''})  #do i need to remap?
...
class TalkingState(smach.StateMachine):
    def __init__(self):
